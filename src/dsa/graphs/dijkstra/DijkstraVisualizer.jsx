...
const DIJKSTRA_CODE_SNIPPET = [
  { id: 1, text: "function dijkstra(startNode, targetNode) {" },
  { id: 2, text: "  let dist = {}; let parent = {};" },
  { id: 3, text: "  let pq = new PriorityQueue();" },
  { id: 4, text: "  dist[startNode] = 0; pq.push({node: startNode, d: 0});" },
  { id: 5, text: "  while (!pq.isEmpty()) {" },
  { id: 6, text: "    let {node: u, d} = pq.pop();" },
  { id: 7, text: "    if (u === targetNode) return reconstructPath(u);" },
  { id: 8, text: "    if (d > dist[u]) continue;" },
  { id: 9, text: "    for (let {node: v, weight} of getNeighbors(u)) {" },
  { id: 10, text: "      let newDist = dist[u] + weight;" },
  { id: 11, text: "      if (newDist < (dist[v] ?? Infinity)) {" },
  { id: 12, text: "        dist[v] = newDist;" },
  { id: 13, text: "        parent[v] = u;" },
  { id: 14, text: "        pq.push({node: v, d: newDist});" },
  { id: 15, text: "      }" },
  { id: 16, text: "    }" },
  { id: 17, text: "  }" },
  { id: 18, text: "  return null;" },
  { id: 19, text: "}" }
];

function* dijkstraGenerator(startNodeId, targetNodeId, edges) {
  yield { line: 1 };
  
  let dist = { [startNodeId]: 0 };
  let parent = {};
  yield { line: 2, dist: {...dist}, parent: {...parent} };
  
  let pq = [{ node: startNodeId, distance: 0 }];
  yield { line: 3, pq: [...pq] };
  
  yield { line: 4, pq: [...pq], dist: {...dist} };
  ...
  while (pq.length > 0) {
    ...
    if (targetNodeId !== null && u === targetNodeId) {
      ...
      yield { line: 7, targetFound: true, finalized: new Set(finalized), currentNode: u, backtrackPath: path };
      return { found: true };
    }
    
    yield { line: 8, currentNode: u };
    if (d > dist[u]) {
      yield { line: 5 }; // Skip to next iteration visually
      continue;
    }
    
    finalized.add(u);
    yield { line: 8, finalized: new Set(finalized), currentNode: u }; // Actually we can just show it evaluated
    ...
    yield { line: 9, neighbors: [...neighbors], currentNode: u };
    
    for (let neighbor of neighbors) {
      let v = neighbor.node;
      let weight = neighbor.weight;
      
      yield { line: 9, checkNeighbor: v, currentNode: u };
      
      let newDist = dist[u] + weight;
      yield { line: 10, checkNeighbor: v, currentNode: u, calcDist: newDist };
      
      let currentDistV = dist[v] !== undefined ? dist[v] : Infinity;
      yield { line: 11, checkNeighbor: v, currentNode: u };
      
      if (newDist < currentDistV) {
        dist[v] = newDist;
        yield { line: 12, dist: {...dist}, checkNeighbor: v, currentNode: u };
        
        parent[v] = u;
        traversedEdges = traversedEdges.filter(e => e.to !== v); // remove old spanning tree edge if exists
        traversedEdges.push({ from: u, to: v });
        yield { line: 13, parent: {...parent}, checkNeighbor: v, currentNode: u, traversedEdges: [...traversedEdges] };
        
        pq.push({ node: v, distance: newDist });
        yield { line: 14, pq: [...pq], checkNeighbor: v, currentNode: u };
        yield { line: 15, checkNeighbor: v, currentNode: u };
      } else {
        yield { line: 15, checkNeighbor: v, currentNode: u };
      }
    }
    yield { line: 16, currentNode: u };
  }
  yield { line: 17 };
  yield { line: 18, finished: true };
  return { found: false };
}
...
const DijkstraVisualizer = () => {
  ...
  const [finalized, setFinalized] = useState(new Set());
  ...
  const initGenerator = () => {
    ...
    resetState();
    generatorRef.current = dijkstraGenerator(startNodeId, target, edges);
  };
  ...
  const stepForward = useCallback(() => {
    ...
    if (value.dist !== undefined) setDist(value.dist);
    if (value.parent !== undefined) setParent(value.parent);
    if (value.finalized !== undefined) setFinalized(value.finalized);
  ...
  const getNodeClass = (id) => {
    ...
    if (finalized.has(id)) return classes.concat("finalized").join(" ");
  ...
  return (
    <div className={`dijkstra-visualizer-container ${isMobile ? 'mobile-view' : ''}`}>
      <div className={`dijkstra-header ${isMobile ? 'flex-col gap-4 text-center' : ''}`}>
        <h1 className={`${isMobile ? 'text-lg flex-wrap' : 'text-2xl'} font-bold tracking-tight text-white flex items-center gap-3 ${isMobile ? 'justify-center w-full' : 'w-max'}`}>
          <span className="text-[#ff6432]">DIJKSTRA</span> VISUALIZER
        </h1>
        
        <div className={`dijkstra-controls w-full flex ${isMobile ? 'flex-col gap-4' : 'justify-between'}`}>
          <div className={`flex flex-wrap gap-4 items-center ${isMobile ? 'justify-center w-full' : ''}`}>
            <div className={`control-group ${isMobile ? 'w-full justify-center' : ''}`}>
              ...
              <button className="dijkstra-btn" onClick={handleScramble}>
                <FaRandom /> Scramble
              </button>
            </div>
            ...
            <div className="control-group relative">
              ...
              <input 
                type="text" 
                className={`dijkstra-input w-16 ${targetSearch !== "" && parseInt(targetSearch) >= nodes.length ? 'border-red-500 text-red-400' : ''}`} 
                placeholder="ID"
                value={targetSearch}
                onChange={(e) => {
            ...
            <div className={`control-group ${isMobile ? 'w-full justify-center' : ''}`}>
              ...
              <input 
                type="range" 
                className={`dijkstra-slider ${isMobile ? 'flex-1' : ''}`}
                min="0.5" 
                max="3" 
                step="0.5" 
                value={playbackSpeed} 
          ...
          <div className={`control-group ${isMobile ? 'w-full justify-center mt-2' : 'ml-auto'}`} style={{background: 'transparent', border: 'none'}}>
            <button className="dijkstra-btn" onClick={() => { initGenerator(); setIsPlaying(false); }}>
              <FaRedo /> Reset
            </button>
            <button className="dijkstra-btn primary" onClick={togglePlay}>
              {isPlaying ? <FaPause /> : <FaPlay />} {isPlaying ? "Pause" : "Play"}
            </button>
            <button className="dijkstra-btn" onClick={executeStep} disabled={isPlaying || targetFound || isFinished}>
              <FaStepForward /> Step
            </button>
          </div>
        </div>
      ...
      <div className="dijkstra-main">
        <div className="dijkstra-graph-container">
          {/* Edges */}
          <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 1 }}>
            {edges.map((edge, i) => {
              ...
              if (isBacktrack) {
                ...
                edgeColor = "#ff6432";
                edgeWidth = "4";
                edgeFilter = "drop-shadow(0 0 5px rgba(255,100,50,0.8))";
              } else if (isTraversed) {
                edgeColor = "rgba(255,100,50,0.5)";
                edgeWidth = "2";
              }
              ...
              return (
                <g key={`edge-${i}`}>
                  ...
                  <text 
                    x={`${midX}%`} y={`${midY}%`} 
                    fill={isChecking || isBacktrack ? "#fff" : "rgba(255,255,255,0.6)"}
                    fontSize="11"
                    fontWeight="bold"
                    textAnchor="middle"
          ...
          {/* Nodes */}
          {nodes.map((node) => {
            const nodeDist = dist[node.id] !== undefined ? dist[node.id] : '∞';
            return (
              <div 
                key={node.id} 
              ...
              >
                ...
                <div className="node-dist">{nodeDist}</div>
        ...
        <div className="dijkstra-side-panel">
          <div className="dijkstra-code-panel">
            {DIJKSTRA_CODE_SNIPPET.map((line) => (
              <div 
                key={line.id} 
          ...
          <div className="dijkstra-state-panel">
            <div>
              <div className="state-title">Priority Queue (Min-Heap)</div>
              <div className="state-row mt-2 min-h-[30px]">
                {pq.length === 0 ? (
                  <span className="text-xs text-gray-600">[Empty]</span>
                ) : (
                  [...pq].sort((a, b) => a.distance - b.distance).map((item, i) => (
                    <div key={`pq-${i}`} className="state-pill queue flex gap-2 border border-[#00ffff]/30">
                      <span>N:{item.node}</span>
                      <span className="text-gray-400">|</span>
                      <span className="text-white font-bold">D:{item.distance}</span>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="mt-2">
              ...
              <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-4 mt-2`}>
                <div className="flex flex-col bg-black/40 p-2 rounded border border-white/5">
                  ...
                  <span className={`text-lg font-bold ${currentNode !== null ? 'text-[#ff6432]' : 'text-gray-700'}`}>
            ...
            {targetFound && (
              <div className="mt-2 p-3 bg-green-500/20 border border-green-500/50 rounded-lg text-green-400 text-center font-bold animate-pulse">
                TARGET NODE {currentNode} FOUND! Shortest Path: {backtrackPath.join(' → ')}
              </div>
            )}
