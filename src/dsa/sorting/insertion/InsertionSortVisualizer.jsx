...
const INSERTION_CODE = [
  { id: 1, text: "for (let i = 1; i < n; i++) {" },
  { id: 2, text: "  let key = arr[i];" },
  { id: 3, text: "  let j = i - 1;" },
  { id: 4, text: "  while (j >= 0 && arr[j] > key) {" },
  { id: 5, text: "    // Highlight Comparison" },
  { id: 6, text: "    arr[j + 1] = arr[j]; // Shift" },
  { id: 7, text: "    j--;" },
  { id: 8, text: "  }" },
  { id: 9, text: "  arr[j + 1] = key; // Insert" },
  { id: 10, text: "}" }
];
...
const InsertionSortVisualizer = () => {
  ...
  const [activeIndices, setActiveIndices] = useState([]); // [j]
  const [shiftingIndices, setShiftingIndices] = useState([]);
  const [markers, setMarkers] = useState({ i: null, j: null, keyIdx: null });
  const [stats, setStats] = useState({ comparisons: 0, shifts: 0 });
  const [calculation, setCalculation] = useState({ left: null, right: null, condition: "", result: null, isShifting: false });
  ...
  const resetStates = () => {
    setActiveIndices([]);
    setShiftingIndices([]);
    setMarkers({ i: null, j: null, keyIdx: null });
    setStats({ comparisons: 0, shifts: 0 });
    setCalculation({ left: null, right: null, condition: "", result: null, isShifting: false });
  ...
  const sort = async () => {
    ...
    pauseRef.current = false;
    addLog("EXEC_START: INSERTION_SORT_ASC");
    ...
    for (let i = 1; i < n; i++) {
      ...
      setMarkers(prev => ({ ...prev, i, keyIdx: i }));
      let key = arr[i];
      setCurrentLine(1);
      await wait(400);
      ...
      while (j >= 0 && arr[j] > key) {
        ...
        // Shift
        addLog(`SHIFT: ${arr[j]} -> index ${j + 1}`);
        setShiftingIndices([j, j + 1]);
        setCalculation(prev => ({ ...prev, isShifting: true }));
        setCurrentLine(6);
        await wait(400);
      ...
      // If loop ends, we still might have checked j >= 0
      if (j >= 0) {
        setStats(s => ({ ...s, comparisons: s.comparisons + 1 }));
      }

      // Insert
      addLog(`INSERT_KEY: ${key} at index ${j + 1}`);
      setShiftingIndices([j + 1]);
      setCalculation({ 
        left: key, 
        right: null, 
        condition: "INSERT", 
      ...
      arr[j + 1] = key;
      setArray([...arr]);
      setMarkers(prev => ({ ...prev, keyIdx: j + 1 }));
      await wait(800);
  ...
  return (
    <div className={`insertion-visualizer-container ${isMobile ? 'mobile-view' : ''}`}>
      <div className={`visualizer-header ${isMobile ? 'flex-col gap-6 items-center text-center' : ''}`}>
        <div className="flex flex-col gap-1">
          <h2 className={`${isMobile ? 'text-lg' : 'text-xl'} font-black text-white flex items-center gap-2`}>
            ...
            INSERTION_SORT <span className="text-[#FFD700]/50 font-light">KERNEL_V1</span>
          </h2>
          <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono">Linear Shift Memory Sync</p>
        </div>
        ...
        <div className={isMobile ? '' : 'lg:col-span-2 relative'}>
          <div className={`sorting-stage ${isMobile ? 'min-h-[220px] py-8 px-2' : ''}`}>
            {array.map((val, idx) => {
              ...
              const isKey = markers.keyIdx === idx;
              const isSortedPart = markers.i !== null && idx < markers.i;
              
              let barState = "";
              if (isShifting) barState = "bar-swapping"; // reuse color
              else if (isComparing) barState = "bar-comparing";
              else if (isKey) barState = "bar-key";
              else if (isSortedPart) barState = "bar-sorted";
          ...
          <div className={`stats-row ${isMobile ? 'mt-16 flex-wrap justify-center gap-4' : 'mt-12'}`}>
            ...
            <div className="stat-item">
              ...
              <span className={`${isMobile ? 'text-sm' : 'text-xl'} font-bold text-[#FFD700]`}>{stats.shifts}</span>
            </div>
            <div className="stat-item">
          ...
          <div className={`calculation-panel ${isMobile ? 'mt-4 py-4' : 'mt-6'}`}>
            ...
            <div className={`flex items-center ${isMobile ? 'gap-3 justify-center' : 'gap-6'}`}>
              ...
              <div className="calc-unit">
                <span className="text-[8px] opacity-40">KEY</span>
              ...
              <div className={`flex flex-col ${isMobile ? 'min-w-[80px]' : 'min-w-[140px]'}`}>
                ...
                <div className="flex flex-col gap-1">
                  <span className={`text-[10px] font-black whitespace-nowrap ${calculation.left === null ? 'text-zinc-600' : (calculation.result ? 'text-[#FFD700]' : 'text-zinc-500')}`}>
                    {calculation.left === null ? 'IDLE' : (calculation.result ? (calculation.condition === 'INSERT' ? 'TRUE (INSERT)' : 'TRUE (SHIFT)') : 'FALSE (STOP)')}
                  </span>
                  {calculation.isShifting && (
