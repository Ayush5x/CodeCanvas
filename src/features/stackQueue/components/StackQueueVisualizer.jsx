...
const StackQueueVisualizer = () => {
  ...
  const vizRef = useRef(null);        // ref for the visualizer canvas box
  const vizScaleRef = useRef(1);      // tracks current DPR scale factor
  ...
  // ── Mobile DPR scaling: fires on mount + resize, no-ops on desktop ──────
  useEffect(() => {
    ...
    const applyScale = () => {
      if (window.innerWidth > 1024) {
        // Desktop: always reset any transform
        el.style.transform = '';
        el.style.transformOrigin = '';
        return;
      }
      const dpr = window.devicePixelRatio || 1;
      const containerW = el.offsetWidth;       // CSS pixels
      const targetW    = window.innerWidth * 0.94; // 94vw target
      // Scale down proportionally when content would exceed target width
      const scale = Math.min(1, targetW / containerW);
      vizScaleRef.current = scale;
      el.style.transform      = `scale(${scale})`;
      el.style.transformOrigin = 'top center';
      // Compensate height so the box doesn't leave a gap
  ...
  <div className="relative z-10">
    ...
    <h1 className={`${isMobile ? 'text-3xl' : 'text-5xl md:text-8xl'} font-black tracking-tighter text-white uppercase italic`}>
    ...
    <p className="mt-6 max-w-lg mx-auto text-zinc-400 text-sm leading-relaxed font-light tracking-wide">
      ...
      <style>{`
        ...
        /* Explanation & DIY Section Styles */
        .explanation-section, .diy-section {
          margin-top: 1rem;
          padding-top: 1.5rem;
          border-top: 1px solid rgba(255, 255, 255, 0.05);
        }
        .explanation-section h3, .diy-section h3 {
          display: flex;
          align-items: center;
          gap: 10px;
          user-select: none;
        ...
        .explanation-content, .diy-content {
          padding: 10px 0;
        }
        .explanation-content ol, .explanation-content ul,
        .diy-content ol, .diy-content ul {
          padding-left: 20px;
          margin: 15px 0;
        }
        ...
        @media (max-width: 1024px) {
          ...
          /* ── Header: vw-based padding + centered ── */
          .os-layout-wrapper > div:first-of-type {
            text-align: center;
            padding-left: 4vw;
            padding-right: 4vw;
          }
          ...
          /* Show visualizer panel above the code panel on mobile */
          .os-grid .os-panel:last-child {
            order: -1;
          }
          ...
          .os-visualizer-box {
            ...
            /* transform applied dynamically by ResizeObserver JS */
          }

          /* Force any raw <canvas> elements to be responsive */
          canvas {
            width: 100% !important;
          ...
          /* ── RUNTIME_SOURCE.C panel: Samsung S24 optimised ── */
          .code-container-scroll pre,
          .code-container-scroll code {
            font-size: 12px !important;          /* legible on S24 360px width  */
            white-space: pre-wrap !important;    /* wrap long lines, no h-scroll */
            overflow-x: hidden !important;       /* eliminate side-scroll        */
          ...
          /* ── Stack: vertical column, centered, no overflow ── */
          .physical-container.stack {
            width: min(130px, 90%);
            min-height: 220px;
          ...
          /* ── Queue: horizontal scroll within bounded width ── */
          .physical-container.queue {
            width: 100%;
            max-width: 100%;
            overflow-x: auto;
          ...
          /* ── Structure info badge: repositioned for mobile ── */
          .structure-info-badge {
            font-size: 9px;
            padding: 4px 8px;
            left: 8px;
          ...
          .code-block {
            padding: 1rem;
            font-size: 11px;
            white-space: pre; /* Maintain indentation but allow scroll */
            border-radius: 6px;
          }
          .docs-panel h3 {
            font-size: 14px;
        ...
        @media (max-width: 640px) {
          ...
          .stack .os-data-node { height: 44px; font-size: 15px; }
          .queue .os-data-node { width: 60px; height: 60px; font-size: 15px; }
        ...
        /* ─── Title font size for phones (< 768px) ───────────────────────── */
        @media (max-width: 767px) {
          .os-layout-wrapper h1 {
      ...
      <div className="os-grid">
        ...
        <motion.div className="os-panel" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }}>
          ...
          <div className="os-visualizer-box" ref={vizRef}>
