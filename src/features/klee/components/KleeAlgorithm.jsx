...
const KleePremiumVisualizer = () => {
  ...
  return (
    ...
    <div style={isMobile ? { ...x.titleContainer, padding: '40px 10px' } : x.titleContainer}>
      <motion.div 
      ...
      >
        ...
        <span style={isMobile ? { ...x.bgWatermark, fontSize: '5rem' } : x.bgWatermark}>
          {isMobile ? 'KLEE' : 'MEASURE'}
        </span>
        
        <motion.h1 
          whileHover={isMobile ? {} : { letterSpacing: '14px', transition: { duration: 0.4 } }}
          style={isMobile ? { ...x.mainTitle, fontSize: '2.5rem', letterSpacing: '2px' } : x.mainTitle}
        >
        ...
        <div style={x.titleUnderline}>
          <motion.div 
            initial={{ width: 0 }}
            animate={{ width: isMobile ? '50px' : '100px' }}
            transition={{ delay: 0.5, duration: 1 }}
            style={x.accentLine} 
          />
          <p style={x.subTitleCaps}>
            UNION MEASURE ENGINE <span style={{ opacity: 0.3 }}>//</span> {isMobile ? 'V1.0' : 'VOL. 1.0'}
          </p>
        </div>
      </motion.div>
    </div>
    ...
    <div style={{position:"relative" , top:"50px"}}>&gt;_</div>
      {/* 1. TOP NAV / CONTROLS */}
      <header style={isMobile ? { ...s.nav, height: 'auto', flexDirection: 'column', gap: '20px', padding: '20px 0' } : s.nav}>
        <div style={s.logo}>KLEE <span style={s.logoAccent}>LABS_</span></div>
        <div style={isMobile ? { ...s.navActions, width: '100%', flexWrap: 'wrap', justifyContent: 'center' } : s.navActions}>
          <button style={isMobile ? { ...s.glassBtn, flex: '1 1 120px' } : s.glassBtn} onClick={generateRandom}>RANDOM</button>
          <button style={isMobile ? { ...s.glassBtn, flex: '1 1 100px' } : s.glassBtn} onClick={reset}>RESET</button>
          <button 
            style={isMobile ? { ...s.primeBtn, flex: '1 1 100%', width: '100%' } : s.primeBtn} 
      ...
      <div style={isMobile ? { ...s.mainGrid, gridTemplateColumns: '1fr', marginTop: '30px' } : s.mainGrid}>
        <aside style={isMobile ? { ...s.sidebar, order: 2 } : s.sidebar}>
          <div style={isMobile ? { ...s.metricCard, padding: '25px', borderRadius: '20px' } : s.metricCard}>
            <label style={s.label}>TOTAL_UNION_MEASURE</label>
            <div style={isMobile ? { ...s.bigNum, fontSize: '2.5rem' } : s.bigNum}>{totalLength.toFixed(1)}<small>px</small></div>
          </div>

          <div style={isMobile ? { ...s.glassCard, padding: '20px', borderRadius: '20px' } : s.glassCard}>
          ...
          <div style={isMobile ? { ...s.instructionBox, padding: '20px', borderRadius: '20px' } : s.instructionBox}>
            <label style={s.label}>NEWBIE_GUIDE</label>
            <p style={s.guideText}>
              <b>Manual Draw:</b> Click and drag on the grid.<br/><br/>
              <b>Randomize:</b> Generate complex overlaps.<br/><br/>
              <b>Visualize:</b> Watch the sweep-line calculate length.
            </p>
          </div>
        </aside>

        <main style={isMobile ? { ...s.canvasContainer, order: 1, height: '400px', borderRadius: '24px' } : s.canvasContainer}>
          <svg 
            ref={svgRef} 
            style={s.svg}
          ...
          >
            <defs>
              <pattern id="dotGrid" width={isMobile ? "20" : "40"} height={isMobile ? "20" : "40"} patternUnits="userSpaceOnUse">
            ...
            {segments.map(seg => (
              <g key={seg.id}>
                <line x1={seg.x1} y1={seg.y} x2={seg.x2} y2={seg.y} stroke="rgba(255,255,255,0.1)" strokeWidth={isMobile ? "8" : "12"} strokeLinecap="round" />
                <line x1={seg.x1} y1={seg.y} x2={seg.x2} y2={seg.y} stroke="#fff" strokeWidth={isMobile ? "3" : "5"} strokeLinecap="round" />
                <circle cx={seg.x1} cy={seg.y} r={isMobile ? "3" : "5"} fill="#fff" />
                <circle cx={seg.x2} cy={seg.y} r={isMobile ? "3" : "5"} fill="#fff" />
              </g>
            ))}
            ...
            {/* Ghost Line (Drawing) */}
            {isDrawing && (
              <line x1={dragStart.x} y1={dragStart.y} x2={currentDragX} y2={dragStart.y} stroke="rgba(255,255,255,0.5)" strokeWidth={isMobile ? "2" : "4"} strokeDasharray="8,4" />
            )}
            ...
            {sweepX && (
              <g>
                ...
                <rect x={sweepX - (isMobile ? 30 : 40)} y="10" width={isMobile ? 60 : 80} height="20" rx="4" fill="#fff" />
                <text x={sweepX} y="24" fill="#000" fontSize={isMobile ? "8" : "10"} fontWeight="900" textAnchor="middle">X: {sweepX.toFixed(0)}</text>
              </g>
            )}
          </svg>
        </main>
