...
import SelectionSortVisualizer from "../../../dsa/sorting/selection/SelectionSortVisualizer";
    ...
    <div className={`os-root ${isMobile ? 'mobile-os' : ''}`}>
      ...
      <div className="page-header-wrap">
        ...
        <div className={`flex flex-col gap-0 ${isMobile ? 'items-center text-center' : ''}`}>
          <span className="text-[10px] font-mono text-zinc-600 mb-2 tracking-tighter">REF: SELECTION_SORT_CORE</span>
          
          <div className={`flex items-baseline ${isMobile ? 'flex-col items-center gap-2' : 'gap-4'}`}>
            <h1 className={`${isMobile ? 'text-3xl' : 'text-5xl'} font-black text-white tracking-widest`}>Selection-Sort</h1>
          ...
          <p className="mt-4 text-zinc-500 text-[11px] uppercase tracking-[0.3em] font-light">
            Incremental minimum selection search engine.
          </p>
        </div>
      </div>
      ...
      <section className="doc-section">
        <div>
          ...
          <p className="doc-text">
            Selection Sort is an in-place comparison sorting algorithm. It has an O(n²) time complexity, which makes it inefficient 
            on large lists, and generally performs worse than the similar insertion sort. Selection sort is noted for its 
            simplicity and it has performance advantages over more complicated algorithms in certain situations.
          </p>
