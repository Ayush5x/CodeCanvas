...
import HeapSortVisualizer from "../../../dsa/sorting/heap/HeapSortVisualizer";
    ...
    <div className={`os-root ${isMobile ? 'mobile-os' : ''}`}>
      ...
      <div className="page-header-wrap">
        ...
        <div className={`flex flex-col gap-0 ${isMobile ? 'items-center text-center' : ''}`}>
          <span className="text-[10px] font-mono text-zinc-600 mb-2 tracking-tighter">REF: HEAP_SORT_CORE</span>
          
          <div className={`flex items-baseline ${isMobile ? 'flex-col items-center gap-2' : 'gap-4'}`}>
            <h1 className={`${isMobile ? 'text-3xl' : 'text-5xl'} font-black text-white tracking-widest`}>Heap-Sort</h1>
          ...
          <p className="mt-4 text-zinc-500 text-[11px] uppercase tracking-[0.3em] font-light">
            Binary tree based memory prioritization and extraction engine.
          </p>
        </div>
      </div>
      ...
      <section className="doc-section">
        <div>
          ...
          <p className="doc-text">
            Heap Sort is a comparison-based sorting algorithm. It can be thought of as an improved selection sort: 
            like selection sort, heap sort divides its input into a sorted and an unsorted region, and it iteratively 
            shrinks the unsorted region by extracting the largest element from it and inserting it into the sorted region.
          </p>
