...
const LINKED_LIST_MASTER_DATA = [
  { id: 1, q: "What is the time complexity to insert a node at the beginning of a Singly Linked List?", options: ["O(1)", "O(n)", "O(log n)", "O(1) if tail is known"], correct: 0, logic: "You only need to update the new node's next pointer and the head pointer, regardless of list size." },
  { id: 2, q: "In a Doubly Linked List, what does each node contain?", options: ["Data and Next", "Data and Prev", "Data, Next, and Prev", "Data only"], correct: 2, logic: "Doubly Linked Lists allow bi-directional traversal by storing both forward and backward pointers." },
  { id: 3, q: "Which of the following is a major disadvantage of Linked Lists compared to Arrays?", options: ["Dynamic size", "Ease of insertion", "No random access", "Memory wastage"], correct: 2, logic: "Linked Lists require O(n) time to access the i-th element because you must traverse from the head." },
  { id: 4, q: "What defines a Circular Linked List?", options: ["It has no head", "The last node points back to the head", "It uses two pointers per node", "Nodes are stored in a circle in memory"], correct: 1, logic: "The 'next' pointer of the tail node is assigned to the head, creating a loop." },
  { id: 5, q: "Which algorithm is commonly used to detect a cycle in a Linked List?", options: ["Binary Search", "Floyd's Cycle-Finding (Tortoise & Hare)", "Dijkstra's", "Quick Sort"], correct: 1, logic: "Using two pointers moving at different speeds will eventually result in them meeting if a loop exists." },
  { id: 6, q: "How much extra space is required for a Singly Linked List of n elements?", options: ["O(1)", "O(n) for pointers", "O(n^2)", "None"], correct: 1, logic: "Each node must store a pointer/reference, adding O(n) overhead compared to a raw array." },
  { id: 7, q: "What is the time complexity to delete a node given only a pointer to that specific node (not the head) in a Singly Linked List?", options: ["O(1)", "O(n)", "O(log n)", "Impossible"], correct: 0, logic: "You can copy the data from the next node into the current node and delete the next node (trick method)." },
  { id: 8, q: "Which data structure is best for implementing a 'Least Recently Used' (LRU) Cache?", options: ["Singly Linked List", "Doubly Linked List + Hash Map", "Static Array", "Binary Tree"], correct: 1, logic: "The Doubly Linked List allows O(1) removal and re-insertion at the head, while the Map provides O(1) lookup." },
  { id: 9, q: "A Header Linked List contains a special node at the:", options: ["Middle", "End", "Beginning", "Every 5th position"], correct: 2, logic: "A header node can store metadata like total list length or a pointer to the last node." },
  { id: 10, q: "To reverse a Singly Linked List, how many pointers are typically used?", options: ["1", "2", "3", "4"], correct: 2, logic: "Usually 'prev', 'current', and 'next' pointers are tracked to flip the links without losing the rest of the list." },
];

const LinkedListQuestion = () => {
  ...
  const startMission = () => {
    const shuffled = [...LINKED_LIST_MASTER_DATA]
      ...
      <AnimatePresence mode="wait">
          ...
          >
            <div className={`flex items-center gap-4 ${isMobile ? 'mb-8' : 'mb-12'}`}>
              ...
              <div>
                ...
                <p className="text-white/30 text-[9px] font-mono tracking-[0.2em]">DOMAIN: LINKED_LISTS_V4</p>
              </div>
            </div>
                  ...
                  >
                    <div>
                      <span className="text-[9px] font-mono text-white/20 uppercase tracking-[0.3em] flex items-center gap-2 mb-3">
                        <Share2 size={10}/> 0x{currentIdx.toString(16).toUpperCase()}
                      </span>
        ...
        {gameState === 'results' && (
          <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`max-w-4xl mx-auto text-center ${isMobile ? 'mt-24' : 'mt-10'}`}>
            ...
            <h1 className={`${isMobile ? 'text-4xl' : 'text-7xl'} font-black italic uppercase tracking-tighter mb-4`}>Mission_Report</h1>
            <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'md:grid-cols-3 gap-6'} mb-12 mt-8`}>
              ...
              <div className="bg-white/[0.02] border border-white/10 p-6 rounded-2xl">
                ...
                <h3 className="text-3xl font-black italic text-white">{accuracy > 85 ? 'MASTER' : accuracy > 50 ? 'EXPERT' : 'INITIATE'}</h3>
              </div>
            </div>
