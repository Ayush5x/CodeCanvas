...
import { 
  ...
  Clock, Hash, Activity, Workflow
} from 'lucide-react';
import Navbar from '../../../components/Navbar';

const FLOWCHART_MASTER_DATA = [
  { id: 1, q: "In the Dijkstra flowchart, what is the first step after initializing distances to infinity?", options: ["Extract minimum node", "Set source distance to 0", "Relax all edges", "Check for cycles"], correct: 1, logic: "You must define a starting point with 0 cost before the priority queue can extract anything." },
  { id: 2, q: "In the A* decision loop, what happens if the 'Current Node' is the 'Goal Node'?", options: ["Continue searching", "Reconstruct and return path", "Reset Open Set", "Calculate h(n)"], correct: 1, logic: "The goal check is the termination condition; if true, the algorithm traces back the parent pointers to show the route." },
  { id: 3, q: "What is the 'Decision Diamond' condition in Dijkstra's relaxation step?", options: ["if (new_dist < current_dist)", "if (node == goal)", "if (edge == negative)", "if (heap == empty)"], correct: 0, logic: "Relaxation only updates a node's distance if the new path found is strictly shorter than the previously known one." },
  { id: 4, q: "In Kruskal's flowchart, what step must occur BEFORE the main loop begins?", options: ["Create a priority queue", "Sort all edges by weight", "Pick a random vertex", "Initialize f(n)"], correct: 1, logic: "Kruskal's is edge-centric and greedy; it requires an ordered list of edges to function efficiently." },
  { id: 5, q: "In the BFS flowchart, when is a node marked as 'Visited'?", options: ["When it is extracted from the queue", "When it is first discovered/enqueued", "When the algorithm finishes", "Only if it's the goal"], correct: 1, logic: "Marking as visited upon discovery prevents the same node from being added to the queue multiple times." },
  { id: 6, q: "What is the primary loop condition for most shortest-path flowcharts?", options: ["While (Open Set is not empty)", "For (i = 0 to V)", "While (Path is not found)", "Until (User stops)"], correct: 0, logic: "The algorithm continues as long as there are potential nodes to explore in the frontier (Open Set)." },
  { id: 7, q: "In Prim's algorithm flow, how are adjacent nodes updated?", options: ["Always update", "Only if edge weight < current key in PQ", "Only if node is already in tree", "By adding h(n)"], correct: 1, logic: "Prim's only updates the 'cheapest connection' value if the new edge is smaller than the best one found so far." },
  { id: 8, q: "In the A* flow, what happens to nodes that have been fully evaluated?", options: ["Deleted from memory", "Moved to Closed Set", "Moved back to Open Set", "Re-indexed"], correct: 1, logic: "The Closed Set ensures we don't waste cycles re-calculating nodes we've already found the optimal path for." },
  { id: 9, q: "Which step is unique to the A* flowchart compared to Dijkstra?", options: ["Extract Min", "Calculate f(n) = g(n) + h(n)", "Relax edges", "Initialize Infinity"], correct: 1, logic: "A* introduces the heuristic $h(n)$ to bias the search toward the destination." },
  { id: 10, q: "In a DSU (Kruskal's) flowchart, what does 'Path Compression' do?", options: ["Shortens the edge weights", "Makes nodes point directly to the root", "Deletes redundant edges", "Speeds up sorting"], correct: 1, logic: "During the 'Find' operation, path compression flattens the tree structure for nearly constant time future lookups." },
];

const FlowchartLogicQuestion = () => {
  ...
  const startMission = () => {
    const shuffled = [...FLOWCHART_MASTER_DATA]
      ...
      <AnimatePresence mode="wait">
        ...
        {gameState === 'config' && (
          ...
          >
            <div className={`flex items-center gap-4 ${isMobile ? 'mb-8' : 'mb-12'}`}>
              <div className="p-3 bg-white/5 rounded-2xl border border-white/10"><Workflow size={isMobile ? 20 : 24} /></div>
              <div>
                <h2 className={`${isMobile ? 'text-xl' : 'text-3xl'} font-black italic uppercase tracking-tighter`}>Flow_Analyzer</h2>
                <p className="text-white/30 text-[9px] font-mono tracking-[0.2em]">DOMAIN: PROCEDURAL_LOGIC_MAPPING</p>
              </div>
            </div>
            ...
            <div className={isMobile ? 'space-y-8' : 'space-y-12'}>
              <section>
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <label className="text-[9px] uppercase tracking-[0.2em] text-white/40">Step_Count</label>
                </div>
            ...
            <div className={`grid grid-cols-1 ${isMobile ? 'gap-2' : 'md:grid-cols-4 gap-4'} mb-8`}>
              ...
              <div className={`${isMobile ? '' : 'md:col-span-2'} bg-white/[0.02] border border-white/10 p-4 rounded-xl`}>
                <div className="flex justify-between items-end mb-2">
                  <p className="text-[8px] uppercase tracking-widest text-white/10 font-mono">Logic_Sequence</p>
            ...
            <div className={`grid ${isMobile ? 'grid-cols-1' : 'lg:grid-cols-3'} gap-8`}>
              ...
              <div className={`bg-white/[0.02] border border-white/10 rounded-[1.5rem] ${isMobile ? 'p-6' : 'p-8'} h-fit backdrop-blur-sm`}>
                ...
                {isLocked ? (
                  ...
                  <p className="text-[9px] font-mono text-white/10 uppercase tracking-widest">Tracing path...</p>
                )}
              </div>
            </div>
          </motion.div>
        ...
        {gameState === 'results' && (
          <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`max-w-4xl mx-auto text-center ${isMobile ? 'mt-24' : 'mt-10'}`}>
            ...
            <h1 className={`${isMobile ? 'text-4xl' : 'text-7xl'} font-black italic uppercase tracking-tighter mb-4`}>Logic_Audit</h1>
            <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'md:grid-cols-3 gap-6'} mb-12 mt-8`}>
              ...
              <div className="bg-white/[0.02] border border-white/10 p-6 rounded-2xl">
                ...
                <h3 className="text-3xl font-black italic text-white">{accuracy > 85 ? 'ARCHITECT' : accuracy > 50 ? 'ENGINEER' : 'ERROR'}</h3>
              </div>
            </div>
...
export default FlowchartLogicQuestion;