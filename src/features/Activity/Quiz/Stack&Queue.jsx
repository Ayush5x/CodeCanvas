...
const STACK_QUEUE_MASTER_DATA = [
  { id: 1, q: "Which principle does a Stack follow?", options: ["FIFO (First In First Out)", "LIFO (Last In First Out)", "LILO (Last In Last Out)", "Random Access"], correct: 1, logic: "Stacks operate like a stack of plates: the last one put on top is the first one taken off (Last In First Out)." },
  { id: 2, q: "Which operation is used to insert an element into a Stack?", options: ["Enqueue", "Add", "Push", "Insert"], correct: 2, logic: "The 'Push' operation adds an element to the top of the stack." },
  { id: 3, q: "What happens when you try to Pop from an empty stack?", options: ["Overflow", "Underflow", "Garbage Value", "Null Pointer"], correct: 1, logic: "Stack Underflow occurs when an operation tries to remove an element from a stack that contains no data." },
  { id: 4, q: "Which principle does a Queue follow?", options: ["LIFO", "FILO", "FIFO (First In First Out)", "Priority Access"], correct: 2, logic: "Queues operate like a line at a store: the first person to arrive is the first to be served." },
  { id: 5, q: "In a Queue, where does insertion (Enqueue) happen?", options: ["At the Front", "At the Rear", "In the Middle", "At the Top"], correct: 1, logic: "Elements are always added to the 'Rear' (tail) of the queue." },
  { id: 6, q: "What is a 'Circular Queue' used for?", options: ["Sorting data", "Efficient memory utilization", "Recursive calls", "Reversing a string"], correct: 1, logic: "Circular queues reuse empty spaces at the front that were created by dequeue operations." },
  { id: 7, q: "Which data structure is used by the compiler to implement recursion?", options: ["Queue", "Stack", "Linked List", "Tree"], correct: 1, logic: "The 'Function Call Stack' keeps track of active subroutines and local variables during recursion." },
  { id: 8, q: "A 'Deque' (Double-Ended Queue) allows insertion/deletion at:", options: ["Only the front", "Only the rear", "Both ends", "The middle only"], correct: 2, logic: "Deques are flexible structures where you can push or pop from both the head and the tail." },
  { id: 9, q: "Which operation removes the front element of a Queue?", options: ["Pop", "Delete", "Dequeue", "Shift"], correct: 2, logic: "Dequeue removes the element from the 'Front' of the queue." },
  { id: 10, q: "What is the time complexity of Push and Pop in a Stack?", options: ["O(n)", "O(log n)", "O(1)", "O(n^2)"], correct: 2, logic: "Since operations only happen at the top, they are constant time O(1)." },
];

const StackQueueQuestion = () => {
  ...
  const startMission = () => {
    const shuffled = [...STACK_QUEUE_MASTER_DATA]
      ...
      <AnimatePresence mode="wait">
          ...
          >
            <div className={`flex items-center gap-4 ${isMobile ? 'mb-8' : 'mb-12'}`}>
              ...
              <div>
                ...
                <p className="text-white/30 text-[9px] font-mono tracking-[0.2em]">DOMAIN: STACK_QUEUE_LOGIC</p>
              </div>
            </div>
            ...
            <div className={isMobile ? 'space-y-8' : 'space-y-12'}>
              <section>
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <label className="text-[9px] uppercase tracking-[0.2em] text-white/40">Question_Payload</label>
                </div>
            ...
            <div className={`grid ${isMobile ? 'grid-cols-1' : 'lg:grid-cols-3'} gap-8`}>
                  ...
                  >
                    <div>
                      <span className="text-[9px] font-mono text-white/20 uppercase tracking-[0.3em] flex items-center gap-2 mb-3">
                        <Layers size={10}/> 0x{currentIdx.toString(16).toUpperCase()}
                      </span>
              ...
              <div className={`bg-white/[0.02] border border-white/10 rounded-[1.5rem] ${isMobile ? 'p-6' : 'p-8'} h-fit backdrop-blur-sm`}>
                ...
                {isLocked ? (
                  ...
                  <p className="text-[9px] font-mono text-white/10 uppercase tracking-widest">Waiting for operation...</p>
                )}
              </div>
            </div>
          </motion.div>
        ...
        {gameState === 'results' && (
          <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`max-w-4xl mx-auto text-center ${isMobile ? 'mt-24' : 'mt-10'}`}>
            ...
            <h1 className={`${isMobile ? 'text-4xl' : 'text-7xl'} font-black italic uppercase tracking-tighter mb-4`}>Final_Audit</h1>
...
export default StackQueueQuestion;