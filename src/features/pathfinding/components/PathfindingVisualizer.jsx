...
const ALGO_OPTIONS = [
  { id: 'Dijkstra', name: 'Dijkstra [Wide Search]' },
  { id: 'AStar', name: 'A* [Focused Heuristic]' }
];
const STYLES = `
  ...
  @media (max-width: 767px) {
    ...
    .node-start::after, .node-end::after {
      font-size: 10px;
    }
  }
`;
      ...
      <header className="glass-nav" style={{marginTop:"50px"}}>
        ...
        <div className="controls-group">
          <div className="relative min-w-[200px]">
            <button
              onClick={() => !isAnimating && setIsDropdownOpen(!isDropdownOpen)}
              className={`w-full bg-black/60 border border-white/10 rounded-xl p-2 px-4 text-sm flex justify-between items-center transition-all ${isAnimating ? 'opacity-50 cursor-not-allowed' : 'hover:border-cyan-400/50'}`}
            >
              <span className="text-cyan-400 font-bold">{ALGO_OPTIONS.find(a => a.id === algo)?.name}</span>
                ...
                >
                  {ALGO_OPTIONS.map((option) => (
                    <button
                      ...
                      className={`w-full p-3 text-left text-sm transition-colors hover:bg-cyan-400/10 ${algo === option.id ? 'text-cyan-400 bg-cyan-400/5' : 'text-zinc-400'}`}
                    >
                      {option.name}
                    </button>
                  ))}
