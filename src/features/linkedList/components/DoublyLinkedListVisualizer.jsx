...
import { FaArrowRight, FaArrowLeft, FaTrash, FaPlus, FaMicrochip, FaCode, FaMemory, FaTerminal, FaLayerGroup } from 'react-icons/fa';
...
const DoublyLinkedListVisualizer = () => {
  ...
  const currentLogic = useMemo(() => ([
    ...
    "2. newNode->next = curr; newNode->prev = curr->prev",
    "3. curr->prev->next = newNode",
    "4. curr->prev = newNode",
    "5. SYNC_DUAL_POINTER_MAP()"
  ]), []);
  ...
  const allocateMemory = useCallback((data) => {
    ...
    return { data, address: MEMORY_ADDRESSES[availableIndex], memoryIndex: availableIndex, next: null, prev: null, isNew: true };
  }, [memoryPool]);
  ...
  const operation = async (type, positionMode = null) => {
    ...
    setIsLoading(true);
    setStatusMsg(`EXEC.DLL_${type.toUpperCase()}`);
    ...
    if (type === 'insert') {
      ...
      if (positionMode === 'start') {
        if (nodes.length > 0) { targetNode.next = nodes[0].memoryIndex; updatedNodes[0].prev = targetNode.memoryIndex; }
        updatedNodes = [targetNode, ...updatedNodes];
      } else if (positionMode === 'end') {
        if (updatedNodes.length > 0) { targetNode.prev = updatedNodes[updatedNodes.length - 1].memoryIndex; updatedNodes[updatedNodes.length - 1].next = targetNode.memoryIndex; }
        ...
        if (index === 0) {
          ...
          targetNode.prev = updatedNodes[index - 1].memoryIndex; updatedNodes[index - 1].next = targetNode.memoryIndex;
          updatedNodes.push(targetNode);
        } else {
          targetNode.next = updatedNodes[index].memoryIndex; targetNode.prev = updatedNodes[index - 1].memoryIndex;
          updatedNodes[index].prev = targetNode.memoryIndex; updatedNodes[index - 1].next = targetNode.memoryIndex;
      ...
      const index = positionMode === 'start' ? 0 : positionMode === 'end' ? nodes.length - 1 : parseInt(positionInput);
      if (isNaN(index) || index < 0 || index >= nodes.length) { triggerError("ERR_BOUNDS"); setIsLoading(false); return; }
      targetNode = updatedNodes[index];
      if (index > 0) updatedNodes[index - 1].next = targetNode.next;
      if (index < updatedNodes.length - 1) updatedNodes[index + 1].prev = targetNode.prev;
  ...
  return (
    <div className="os-container">
      <style>{`
        ...
        /* Prevent blue click highlight on mobile */
        ...
        @media (min-width: 1280px) { .main-layout { grid-template-columns: 380px 1fr 340px; } }
        ...
        .brand { font-size: 1.5rem; font-weight: 700; margin-bottom: 25px; border-left: 5px solid var(--accent); padding-left: 15px; letter-spacing: -1px; }
        .label { font-size: 10px; color: var(--accent); font-weight: bold; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 12px; display: block; opacity: 0.8; }
        
        .input-row { display: flex; gap: 12px; margin-bottom: 20px; }
        input { flex: 1; background: rgba(255,255,255,0.06); border: 1px solid var(--glass-border); padding: 16px; color: white; border-radius: 12px; outline: none; width: 0; min-width: 80px; }
        input:focus { border-color: var(--accent); background: rgba(255,255,255,0.1); }

        .btn-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        button { background: rgba(255,255,255,0.03); border: 1px solid var(--glass-border); color: var(--text-primary); padding: 14px; border-radius: 12px; cursor: pointer; font-size: 11px; font-weight: 700; transition: 0.3s; white-space: nowrap; }
        ...
        .ptr-row { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 8px; font-size: 7px; color: #00ffcc; font-weight: bold; text-align: left; }
        
        .connector { display: flex; flex-direction: column; align-items: center; gap: 8px; margin: 0 15px; opacity: 0.4; color: white; font-size: 18px; }
        ...
        .ram-slot.active { background: linear-gradient(135deg, var(--magenta), #8000ff); border-color: transparent; color: white; box-shadow: 0 0 20px rgba(255,0,255,0.4); }
    ...
    <div className="relative z-10 flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-4">
        <h1 className="text-7xl font-light tracking-tighter text-white">
          Double<span className="font-black text-[25px]  text-[#00ffcc]">_List</span>
        </h1>
      </div>

      <p className="max-w-md text-zinc-400 text-sm leading-relaxed border-l-2 border-zinc-800 pl-4">
        Bi-directional node linkage allowing seamless forword and backword navigation . Critical for undo-redo systems .
      </p>
    </div>
  </div>
    ...
    <div className="flex items-baseline justify-end gap-4">
      ...
      <div className="main-layout">
        <aside className="panel">
          <div className="brand">DOUBLY<span>_LIST.OS</span></div>
          ...
          <div className="btn-grid">
            <button onClick={() => operation('insert', 'start')}><FaPlus /> PUSH_FRONT</button>
            <button onClick={() => operation('insert', 'end')}><FaPlus /> PUSH_BACK</button>
            <button style={{ gridColumn: 'span 2' }} onClick={() => operation('insert', 'pos')}><FaLayerGroup /> INSERT_AT_POSITION</button>
            <button onClick={() => operation('delete', 'start')}><FaTrash /> POP_FRONT</button>
            <button onClick={() => operation('delete', 'end')}><FaTrash /> POP_BACK</button>
            <button style={{ gridColumn: 'span 2' }} onClick={() => operation('delete', 'pos')}><FaTrash /> DELETE_AT_POSITION</button>
          </div>
          <div style={{ marginTop: 'auto', paddingTop: '30px' }}>
            <span className="label">Velocity: {animationSpeed}ms</span>
            <input type="range" style={{width:'100%', accentColor:'white'}} min="100" max="1500" value={animationSpeed} onChange={e => setAnimationSpeed(parseInt(e.target.value))} />
          </div>
        </aside>
            ...
            <div key={node.address} style={{display: 'flex', alignItems: 'center'}}>
              <div className="node-box">
                ...
                <div className="ptr-row">
                  <div>P: {node.prev !== null ? MEMORY_ADDRESSES[node.prev] : 'NULL'}</div>
                  <div>N: {node.next !== null ? MEMORY_ADDRESSES[node.next] : 'NULL'}</div>
                </div>
              </div>
              {i < nodes.length - 1 && (
      ...
      <footer className="panel">
        <span className="label"><FaMemory /> Physical_RAM_Allocation</span>
